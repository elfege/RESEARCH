...
Three lines of elementary arithmetic suffice to expose the structure of the Hegelian Aufhebung:
...
**First line: abstract negation.** Affirmation meets its negation. The result is annihilation. This is contradiction as the understanding (*Verstand*) conceives it: two opposed terms cancel each other. Nothing remains.

**Second line: negation preserved.** The same terms, but negation is now *preserved* as magnitude through absolute value. The result is no longer zero but two — more than the starting point. Contradiction, instead of destroying, *produces*. This marks the passage from understanding to reason (*Vernunft*): not suppressing the negative, but passing through it.

**Third line: the decisive identity.** The absolute value of the negative equals the square of the negative. And the square of (-1) is nothing other than (-1) × (-1) — negation entering into a *relation* (*Verhältnis*) with itself. The negative passes through an other that is itself.

This is where Aufhebung reveals its barest structure:

- **|-1|** removes the negative sign. The operation is *immediate*: negation disappears, magnitude is preserved. The understanding cleans up.
- **(-1)²** traverses negation through itself. The operation is *mediated*: the negative relates to itself through its other (which is itself), and it is this passage — this relation — that produces the positive.

Both operations yield the same result. But one *conceals* the mediation, the other *shows* it. Absolute value is Aufhebung as seen by the understanding; squaring is Aufhebung as seen by reason.
...
The relation (*Verhältnis*) is not a means toward mediation. It *is* the mediation. In (-1) × (-1), the multiplication is the relation itself — the operation through which the negative enters into relation with its other (itself) and produces. Without relation, no passage. The result (1) is not given in advance; it is *engendered* by the relation.

This is why Aufhebung is not a subtraction, nor an annihilation, nor even a "synthesis" in the textbook sense. It is a productive relation of negation with itself.

---

### Conclusion

These three lines do not demonstrate Aufhebung. They *clarify* it — by showing it at work in an operation that every schoolchild knows without grasping its speculative import. Absolute value is the arithmetic Aufhebung: the suppression-preservation of the negative, identical to the result of the negation of negation.

The clarification *is* the contribution. Hegel devoted hundreds of pages to expounding this movement. Arithmetic exposes it in three lines — not against Hegel, but as an illustration of what he was trying to say: that dialectical structure is not a philosophical invention, but a universal form inscribed in the most elementary real.
